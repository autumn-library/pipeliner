...
// Зарезервированные стадии пайплайна, не попадающие в stages
//
//  Возвращаемое значение:
//   Соответствие - КлючИЗначение:
//                    * Ключ - Строка - имя обёртки на ОписаниеЗадачи
//                    * Значение - Строка - имя зарезервированной стадии
//
Функция ЗарезервированныеСтадии() Экспорт

	Стадии = Новый Соответствие();
	
	Стадии.Вставить("ДоВсех", ".pre");
	Стадии.Вставить("ПослеВсех", ".post");

	Возврат Стадии;

КонецФункции

// Предопределённое имя джобы публикации GitLab Pages
//
//  Возвращаемое значение:
//   Строка - pages
//
Функция ИмяЗадачиСтраниц() Экспорт
