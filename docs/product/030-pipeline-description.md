...
Массив `stages` генератор собирает автоматически из стадий задач (порядок появления). Зарезервированные стадии `.pre`/`.post` в список не попадают — подробнее в разделе [Стадии](stages.md).
