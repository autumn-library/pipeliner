...
Расширенный вариант (exclude/project/ref) — через `НовыйПравило()`; тогда exists выводится объектом с `paths`:

```bsl
Правило = Задача.НовыйПравило();
Правило.Существует.Добавить("**/*.os");
Правило.СуществуетИсключить.Добавить("tests/**");
...
- exists задан через exclude/project/ref без путей;
